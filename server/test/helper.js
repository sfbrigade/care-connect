...
async function buildPostgres (t) {
  ...
  // --skip-generate: the client is already generated by the `pretest` hook.
  // Without this, every test process regenerates node_modules/.prisma/client
  // concurrently, racing on the package.json rewrite and producing flaky
  // `Cannot find module '#main-entry-point'` failures in sibling processes.
  await util.promisify(execFile)('npx', ['prisma', 'db', 'push', '--schema', schemaPath, '--skip-generate'], {
