...
describe('Care flow unit tests', () => {
  ...
  it('groups not-in-custody records into Still onsite / Exited facility / Transferred to jail / Transferred to hospital', () => {
    const grouped = groupCareNotInCustodySections([
      ...
      { id: 4, subjectStatus: 'EXITED', exitDestinationId: 'hospital', releasedAt: null },
    ]);
    ...
    expect(grouped.TRANSFERRED_TO_HOSPITAL.map(d => d.id)).toEqual([4]);
  });
