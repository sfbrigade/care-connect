 ...
 * @param {object} deflection - Deflection object with subjectStatus and admittedAt
...
export function releaseTiming (deflection, now = DateTime.now()) {
  ...
  if (!deflection?.admittedAt) return null;

  const admitted = typeof deflection.admittedAt === 'string'
    ? DateTime.fromISO(deflection.admittedAt)
    : DateTime.fromJSDate(deflection.admittedAt);
  if (!admitted.isValid) return null;

  const hoursSinceAdmission = now.diff(admitted, 'hours').hours;
