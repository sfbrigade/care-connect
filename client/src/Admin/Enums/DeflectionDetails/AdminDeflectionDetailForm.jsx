...
function AdminDeflectionDetailForm() {
    ...
    const form = useForm({
        ...
        initialValues: {
            id: '',
            name: '',
            deflectionDetailCategoryName: ''
        },
        validate: {
            id: isNotEmpty('ID (Slug) is required.'),
            name: isNotEmpty('Name is required.'),
            deflectionDetailCategoryName: isNotEmpty('Detail Category is required.'),
        },
    });
    ...
    const onSubmitMutation = useMutation({
        mutationFn: (values) => {
            if (isNew) {
                values.deflectionDetailCategoryId = detailCategories.filter((x) => x.name === values.deflectionDetailCategoryName)[0].id;
    ...
    const reopenMutation = useMutation({
        mutationFn: () => Api.deflections.details.reopen(detailId),
        onError: (errors) => form.setErrors(errors),
        onSuccess: () => {
            navigate('/admin/enums/deflection-details');
        },
                ...
                <form onSubmit={form.onSubmit((values) => onSubmitMutation.mutateAsync(values))}>
                    <Fieldset disabled={isLoading} variant='unstyled'>
                        <Stack>
                            ...
                            <Select
                                {...form.getInputProps('deflectionDetailCategoryName')}
                                key={form.key('deflectionDetailCategoryName')}
                                label='Detail Category'
                                data={detailCategories?.map((x) => x.name)}
                            ...
                            <Group>
                                ...
                                {!isNew && (
                                    <>
                                        <Button variant="light" onClick={() => reopenMutation.mutate()}
                                                disabled={onSubmitMutation.isPending}>
                                            Reopen
                                        </Button>
                                        <Button
