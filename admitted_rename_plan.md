...
This is broader than a simple enum rename. The current codebase also exposes `admittedAt`, `admittedById`, and `admittedBy` through Prisma, server response schemas, frontend helpers, status/capacity reporting, seeds, tests, stories, and docs.
...
I do not recommend changing only the enum first and leaving field renames for later unless there is a strong need for a staged rollout. The current frontend and tests are tightly coupled to both the status string and the `admitted*` field names, so a partial rename would create temporary contract mismatches.
...
- `User.deflectionsAdmitted` -> `deflectionsMedicalIntakeStarted` or similarly explicit relation name
- relation name `"AdmittedBy"` -> `"BeginMedicalIntakeBy"` or similarly explicit relation name
...
- Update `server/routes/api/status/capacity.js` to decide whether the external payload should also rename `admittedAt`.

Recommendation for `/api/status/capacity`:

- Treat it as part of the contract rename and change the payload to `beginMedicalIntakeAt`.
- If an external dashboard depends on `admittedAt`, either:
  - coordinate the consumer change in the same release, or
  - temporarily return both fields during a short compatibility window.

That compatibility decision should be made before implementation, because it affects both route schema and tests.
...
- `client/src/utils/releaseTiming.js` and `client/src/lesc/components/holdsViewModel.js` currently rely on `admittedAt`. If the API field is renamed, these helpers must move with it or the UI will fail silently.
...
4. Decide and implement whether capacity output has a compatibility window for `admittedAt`.
...
- Is this a hard rename everywhere, or do any external consumers require temporary support for old field names?

This matters most for:

- `server/routes/api/status/capacity.js`
- any frontend code deployed separately from the API

### Naming consistency

The proposed field names are clear, but relation naming should also be made explicit. Recommended names:

- `beginMedicalIntakeAt`
- `beginMedicalIntakeById`
- `beginMedicalIntakeBy`
- `deflectionsMedicalIntakeStarted`
- relation name such as `"BeginMedicalIntakeBy"`
...
6. Timing-based UI that previously used `admittedAt` still works with the new field.
7. Status-capacity output matches the intended final contract.
...
The main revision to the original analysis is that `server/models/deflection.js`, `server/routes/api/status/capacity.js`, several shared client helpers, and a wider set of tests/stories/docs must be included. The only open design question before implementation is whether any consumer needs a temporary compatibility window for `admittedAt`-style API fields.
